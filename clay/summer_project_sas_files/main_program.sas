...
DATA customer_seeds(KEEP = seed);
    FORMAT seed $1500.;
    SET customer_rails;

    IF fishy = 1     THEN t_fishy = 't';     ELSE t_fishy = 'f';
    IF dead  = 1     THEN t_dead  = 't';     ELSE t_dead  = 'f';
    IF valid_zip = 1 THEN t_valid_zip = 't'; ELSE t_valid_zip = 'f';
    IF dup_ssn = 1   THEN t_dup_ssn = 't';   ELSE t_dup_ssn = 'f';
    IF death_date_est = . THEN t_death_date_est = 'Date.new(1900,1,1)'; 
            ELSE t_death_date_est = 'Date.new(' || trim(put(year(death_date_est),4.)) || ','
    ...
    seed =   '{:cust_id => "'  || trim(cust_id)  || '", '
           ...
           || ':death_date_est => ' || t_death_date_est || ', '
           || ':death_age => ' || trim(put(death_age,6.2)) || ', '
           || ':death_age_group => "' || trim(death_age_group) || '", '
           || ':death_reward_r  => ' || trim(put(death_reward_r,6.)) || ', '
           || ':death_reward_cat => ' || trim(put(death_reward_cat,3.)) || ', '
           || ':cctype => "' || trim(cctype) || '", '
           ...
           || ':death_adjuster => ' || trim(death_adjuster) || ', '
           || ':death_technician => ' || trim(death_technician) || ', '
           || ':death_adj_city => ' || trim(death_adj_city) || ', '
           || ':death_adj_state => ' || trim(death_adj_state) || ', '
           || ':death_cust_adj_dist => ' || trim(put(death_cust_adj_dist,8.)) || ', '
...
DATA _NULL ;         /* No SAS data set is created */ 
    SET customer_seeds ; 
    FILE  'c:\iaa\export\seeds.rb' ;    /* Output Text File */ 
    PUT seed /; 
run ;

/* TODO: Fix . values for missing numbers */
...
/* Organize cities and states for export */
/*PROC SORT DATA = customer_consolidated;*/
/*    BY State City;*/
/*RUN;*/
/**/
/*DATA cities(KEEP = city_name state_abbr);*/
/*    SET customer_consolidated(RENAME=(City  = city_name*/
/*                                      State = state_abbr));*/
/*    BY  state_abbr city_name;*/
/*    IF  FIRST.city_name THEN OUTPUT;*/
/*RUN;*/
/**/
/*PROC SORT DATA = cities;*/
/*    BY state_abbr;*/
...
/*DATA states;*/
/*    SET states;*/
/*    FORMAT state_name $ 12.*/
/*           state_fips 3.;*/
/**/
/*    SELECT;*/
/*        WHEN (state_abbr = 'AK') state_name = 'Alaska';*/
/*        WHEN (state_abbr = 'CA') state_name = 'California';*/
/*        WHEN (state_abbr = 'HI') state_name = 'Hawaii';*/
/*        WHEN (state_abbr = 'NV') state_name = 'Nevada';*/
/*        WHEN (state_abbr = 'OR') state_name = 'Oregon';*/
/*        WHEN (state_abbr = 'WA') state_name = 'Washington';*/
/*    END;*/
/**/
/*    SELECT;*/
/*        WHEN (state_abbr = 'AK') state_fips = 2;*/
/*        WHEN (state_abbr = 'CA') state_fips = 6;*/
/*        WHEN (state_abbr = 'HI') state_fips = 15;*/
/*        WHEN (state_abbr = 'NV') state_fips = 32;*/
/*        WHEN (state_abbr = 'OR') state_fips = 41;*/
/*        WHEN (state_abbr = 'WA') state_fips = 53;*/
/*    END;*/
/*RUN;*/
/**/
/**/
...
/*RUN;*/
/**/
/*DATA cities(DROP = state_name);*/
/*    MERGE cities (IN = inCit)*/
/*          states (IN = inSta);*/
/*    BY state_abbr;*/
/*    IF inCit;*/
/*RUN;*/
/**/
/*/* Give each city a unique ID number */*/
/*DATA cities;*/
/*    SET cities;*/
/*    FORMAT id 4.;*/
/*    id = _N_;*/
...
/*RUN;*/
/**/
/*DATA customer_consolidated(RENAME=(id = city_id));*/
/*    MERGE customer_consolidated (IN = inCust)*/
/*          cities(RENAME = (city_name = City state_abbr = State) IN = inCit);*/
/*    BY    State City;*/
/*    IF    inCust;*/
/*RUN;*/
/**/
/*/* What I want to do now is to determine which county each city is in.*/
/*   Merge Cities with zipcode_13q3_unique and keep the county fips & name */*/
/**/
/*PROC SORT DATA = Zipcode_13q3_unique FORCE;*/
/*    BY STATECODE CITY;*/
/*RUN;*/
/**/
/*DATA zipnew(KEEP = STATECODE CITY COUNTY COUNTYNM CITY);*/
/*    SET Zipcode_13q3_unique;*/
/*RUN;*/
/**/
/*PROC DATASETS LIB = work NOLIST;*/
/*   MODIFY zipnew;*/
...
/*RUN;*/
/**/
/*PROC SORT DATA = zipnew FORCE;*/
/*    BY STATECODE CITY;*/
/*RUN;*/
/**/
/*DATA cities(KEEP = id city_name state_fips state_abbr county countynm);*/
/*    MERGE cities (IN = inCit)*/
/*          zipnew (RENAME = (STATECODE = state_abbr CITY = city_name));*/
/*    BY    state_abbr city_name;*/
/*    IF    inCit;*/
/*RUN;*/


/* Each unique city needs a city_id.
  Merge this back with the customer_consolidated table



/*/* Put (presumably) dead customers into a data set */*/
/*DATA cust_consol_dead cust_consol_alive;*/
/*    SET customer_consolidated;*/
/*    IF      Dead = 1 THEN OUTPUT cust_consol_dead;*/
/*    ELSE IF Dead = 0 THEN OUTPUT cust_consol_alive;*/
/*RUN;*/


/*/* Take a smaller sample to work with */*/
/*PROC SURVEYSELECT DATA     = cust_consol_dead */
/*                  METHOD   = SRS */
/*                  REP      = 1 */
/*                  SAMPSIZE = 4000*/
/*                  SEED     = 8675309*/
...
/*RUN;*/
/**/
/*PROC REG DATA = sample_dead;*/
/*    PredDeath: MODEL Grand_avg_Reward_A = bad_health_index;*/
/*RUN;*/
/*QUIT;*/
/**/
/*PROC SORT DATA = customer_consolidated;*/
...
/* Test Regression */
/*PROC CORR DATA = cust_consol_dead PLOTS(only) = SCATTER(NVAR = ALL ELLIPSE = NONE) ;*/
/*    VAR Death_Age Bad_Health_Index;*/
/*    */
/*RUN;*/
/**/
/**/

/* Huh. The more somebody smokes, the longer they live? */
/*DATA dead_smokers;*/
/*    SET cust_consol_dead(WHERE=(Tobacco_Num_Score > 0));*/
/*RUN;*/
/**/
/*PROC REG DATA = dead_smokers;*/
/*    PredDeath: MODEL Death_Age = Tobacco_Num_Score;*/
/*RUN;*/
/*QUIT;*/

/* Looking at Cancer */
/*PROC REG DATA = cust_consol_dead PLOTS(ONLY MAXPOINTS = 50000) = DIAGNOSTICS(UNPACK);*/
/*    CancerDeaths: MODEL Death_Age = Med_HA_Score bmi_prime;*/
/*RUN;*/
/*QUIT;*/
/**/
/*PROC CORR DATA = cust_consol_dead PLOTS(ONLY MAXPOINTS = 500000) = SCATTER;*/
/*    VAR Death_Age bmi;*/
/*RUN;*/
/**/
...
/* Stuff for future consideration */

/* Add the zip city and state for adjusters to make it
   easier to see where they are travelling from */

/* Would find customers with multiple policies of
   the same type, but there aren't any */

/*DATA cust_mult;*/
/*    SET policyTypeCounts;*/
/*    WHERE whole_count > 1 OR */
/*          term_count  > 1 OR*/
/*          variable_count > 1;*/
/*RUN;*/

/* This person definitely is committing fraud */

/*DATA fraud;*/
/*    SET customer_transactions;*/
/*    WHERE Cust_ID = 'PSX000991081';*/
/*RUN;*/

/* Rank the zip codes in terms of highest average income.
   Also rank them by lowest percentage of policies rewarded.
   Look for term policies: they expire without payout. 
   As income goes up, policy coverage amount goes up. These
   customers presumably are paying higher premiums. Look for
   customers with high income and term policies. */
