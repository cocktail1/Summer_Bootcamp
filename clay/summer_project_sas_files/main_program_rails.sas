...
DATA customer_medical_risk (DROP =  MedF_HA  
    ...
    IF MedF_HA   = 'Y' THEN Med_HA_Score   + (&factor_HA   * 0.5);
    IF MedF_BP   = 'Y' THEN Med_BP_Score   + (&factor_BP   * 0.5);
    IF MedF_Can  = 'Y' THEN Med_Can_Score  + (&factor_Can  * 0.5);
    IF MedF_Diab = 'Y' THEN Med_Diab_Score + (&factor_Diab * 0.5);
    IF MedF_Chol = 'Y' THEN Med_Chol_Score + (&factor_Chol * 0.5);
    IF MedF_Arth = 'Y' THEN Med_Arth_Score + (&factor_Arth * 0.5);
    IF MedF_Asth = 'Y' THEN Med_Asth_Score + (&factor_Asth * 0.5);
    IF MedF_Gla  = 'Y' THEN Med_Gla_Score  + (&factor_Gla  * 0.5);
    IF MedF_Kid  = 'Y' THEN Med_Kid_Score  + (&factor_Kid  * 0.5);
    IF MedF_Leuk = 'Y' THEN Med_Leuk_Score + (&factor_Leuk * 0.5);
    IF MedF_Ment = 'Y' THEN Med_Ment_Score + (&factor_Ment * 0.5);
    IF MedF_SE   = 'Y' THEN Med_SE_Score   + (&factor_SE   * 0.5);
    IF MedF_SCA  = 'Y' THEN Med_SCA_Score  + (&factor_SCA  * 0.5);
    IF MedF_Str  = 'Y' THEN Med_Str_Score  + (&factor_Str  * 0.5);
    IF MedF_TD   = 'Y' THEN Med_TD_Score   + (&factor_TD   * 0.5);
    IF MedF_TB   = 'Y' THEN Med_TB_Score   + (&factor_TB   * 0.5);
    IF MedF_Ul   = 'Y' THEN Med_Ul_Score   + (&factor_Ul   * 0.5);

    IF MedM_HA   = 'Y' THEN Med_HA_Score   + (&factor_HA   * 0.5);
    IF MedM_BP   = 'Y' THEN Med_BP_Score   + (&factor_BP   * 0.5);
    IF MedM_Can  = 'Y' THEN Med_Can_Score  + (&factor_Can  * 0.5);
    IF MedM_Diab = 'Y' THEN Med_Diab_Score + (&factor_Diab * 0.5);
    IF MedM_Chol = 'Y' THEN Med_Chol_Score + (&factor_Chol * 0.5);
    IF MedM_Arth = 'Y' THEN Med_Arth_Score + (&factor_Arth * 0.5);
    IF MedM_Asth = 'Y' THEN Med_Asth_Score + (&factor_Asth * 0.5);
    IF MedM_Gla  = 'Y' THEN Med_Gla_Score  + (&factor_Gla  * 0.5);
    IF MedM_Kid  = 'Y' THEN Med_Kid_Score  + (&factor_Kid  * 0.5);
    IF MedM_Leuk = 'Y' THEN Med_Leuk_Score + (&factor_Leuk * 0.5);
    IF MedM_Ment = 'Y' THEN Med_Ment_Score + (&factor_Ment * 0.5);
    IF MedM_SE   = 'Y' THEN Med_SE_Score   + (&factor_SE   * 0.5);
    IF MedM_SCA  = 'Y' THEN Med_SCA_Score  + (&factor_SCA  * 0.5);
    IF MedM_Str  = 'Y' THEN Med_Str_Score  + (&factor_Str  * 0.5);
    IF MedM_TD   = 'Y' THEN Med_TD_Score   + (&factor_TD   * 0.5);
    IF MedM_TB   = 'Y' THEN Med_TB_Score   + (&factor_TB   * 0.5);
    IF MedM_Ul   = 'Y' THEN Med_Ul_Score   + (&factor_Ul   * 0.5);
...
/* Build customer table for Rails */

DATA customer_rails (KEEP = Cust_ID
                            fishy
                            Dead
                            gender
                            race
                            ...
                            caffeine_score
                            alcohol_score
                            Grand_avg_Cov_Inc_Ratio
                            Grand_Cov_Limit
                            Grand_Reward_A);
    SET customer_consolidated;
RUN;

PROC SORT DATA = customer_rails;
    BY cust_best_zip;
RUN;

/* subset and only keep the city_id */
/*DATA unique_zips_subset(DROP = city_name state_fips state_abbr state_name county_fips county_name);*/
/*    SET unique_zips;*/
/*RUN;*/
/**/
/*PROC SORT DATA = unique_zips_subset;*/
/*    BY cust_best_zip;*/
/*RUN;*/
/**/
/*DATA customer_rails;*/
/*    MERGE customer_rails     (IN = inRails)*/
/*          unique_zips_subset (IN = inZips);*/
/*    BY    cust_best_zip;*/
/*    IF    inRails;*/
/*RUN;*/
/**/
/*DATA customer_rails(DROP = givenName middleInitial surname);*/
/*    FORMAT name $30.;*/
/*    SET customer_rails;*/
/**/
/*    name = TRIM(TRIM(givenName)||' '||TRIM(middleInitial)||'. '||TRIM(surname));*/
/*RUN;*/
/**/
/**/
/*DATA ref_zip(KEEP = zip city_id county_fips state_fips);*/
/*    SET unique_zips(RENAME=(cust_best_zip = zip));*/
/*RUN;*/
/**/
/*PROC SORT DATA = ref_zip;*/
/*    BY zip;*/
/*RUN;*/
/**/
/*PROC SORT DATA = cities;*/
...
/*RUN;*/
/**/
/*DATA seeds_zip(KEEP=seed);*/
/*    FORMAT seed $300.;*/
/*    SET ref_zip;*/
/**/
/*    seed = cat( '{:zip => "',TRIM(PUT(zip,Z5.)), '", ',*/
/*                 ':state_fips => ', TRIM(PUT(state_fips,3.)), ', ',*/
/*                 ':county_fips => ', TRIM(PUT(county_fips, 5.)), ', ',*/
/*                 ':city_id => ', TRIM(PUT(city_id,5.)), '},');*/
/*RUN; */
/**/
/*DATA _NULL_ ;*/
/*    * IF _N_ < 50 THEN DO;*/
/*        SET seeds_zip nobs=nobs; */
/*        FILE  'c:\iaa\export\zip_seeds.rb' LRECL = 1050;*/
/*        IF _N_ = 1 THEN PUT 'zips = Zip.create ([' ;*/
/**/
/*        PUT seed; */
/**/
...
/*RUN;*/
/**/
/*DATA seeds_cities(KEEP = seed);*/
/*    FORMAT seed $300.;*/
/*    SET cities;*/
/**/
/*    seed = CAT( '{:city_id => ', TRIM(PUT(city_id, 5.)), ', ',*/
...
/*DATA _NULL_ ;*/
/*    * IF _N_ < 50 THEN DO;*/
/*        SET seeds_cities nobs=nobs; */
/*        FILE  'c:\iaa\export\city_seeds.rb' LRECL = 1050;*/
/*        IF _N_ = 1 THEN PUT 'cities = City.create ([' ;*/
/**/
/*        PUT seed; */
/**/
/*        IF _N_ = nobs THEN PUT / ' ])';*/
...
/*RUN;*/
/**/
/*DATA seeds_states(KEEP= seed);*/
/*    FORMAT seed $300.;*/
/*    SET states;*/
/**/
/*    seed = CAT ('{:state_fips => ', TRIM(PUT(state_fips, 3.)), ', ',*/
...
/*DATA _NULL_ ;*/
/*    * IF _N_ < 50 THEN DO;*/
/*        SET seeds_states nobs=nobs; */
/*        FILE  'c:\iaa\export\state_seeds.rb' LRECL = 1050;*/
/*        IF _N_ = 1 THEN PUT 'states = State.create ([' ;*/
/**/
/*        PUT seed; */
/**/
/*        IF _N_ = nobs THEN PUT / ' ])';*/
...
/*RUN;*/
/**/
/*DATA seeds_counties(KEEP = seed);*/
/*    FORMAT seed $300.;*/
/*    SET counties;*/
/**/
/*    seed = CAT( '{:county_name => "', TRIM(county_name), '", ', */
/*                 ':county_fips => ',  TRIM(PUT(county_fips,5.)), ', ',*/
/*                 ':state_fips => ',   TRIM(PUT(state_fips, 5.)), '}, ' );*/
/*RUN;*/
/**/
/*DATA _NULL_ ;*/
/*    * IF _N_ < 50 THEN DO;*/
/*        SET seeds_counties nobs=nobs; */
/*        FILE  'c:\iaa\export\county_seeds.rb' LRECL = 1050;*/
/*        IF _N_ = 1 THEN PUT 'counties = County.create ([' ;*/
/**/
/*        PUT seed; */
/**/
/*        IF _N_ = nobs THEN PUT / ' ])';*/
...
/*RUN;*/
/**/
/*DATA seeds_bmi(KEEP = seed);*/
/*    FORMAT seed $300.;*/
/*    SET    codes_bmi;*/
/**/
/*    seed = CAT( '{:code_id => ', TRIM(PUT(code_id,3.)), ', ',*/
/*                 ':code_name => "', TRIM(code_name), '"},');*/
/*RUN;*/
/**/
/*DATA _NULL_ ;*/
/*    * IF _N_ < 50 THEN DO;*/
/*        SET seeds_bmi nobs=nobs; */
/*        FILE  'c:\iaa\export\bmi_seeds.rb' LRECL = 1050;*/
/*        IF _N_ = 1 THEN PUT 'bmicodes = Bmicode.create ([' ;*/
/**/
/*        PUT seed; */
/**/
/*        IF _N_ = nobs THEN PUT / ' ])';*/
...
/*RUN;*/
/**/
/*DATA seeds_reward(KEEP = seed);*/
/*    FORMAT seed $300.;*/
/*    SET    codes_reward;*/
/**/
/*    seed = CAT( '{:code_id => ', TRIM(PUT(code_id,3.)), ', ',*/
/*                 ':code_name => "', TRIM(code_name), '", ',*/
/*                 ':code_exclusion => ', IFC(code_exclusion = 1, 'true', 'false'), '},');*/
/*    RUN;*/
/**/
/*DATA _NULL_ ;*/
/*    * IF _N_ < 50 THEN DO;*/
/*        SET seeds_reward nobs=nobs; */
/*        FILE  'c:\iaa\export\reward_seeds.rb' LRECL = 1050;*/
/*        IF _N_ = 1 THEN PUT 'rewardcodes = Rewardcode.create ([' ;*/
/**/
/*        PUT seed; */
/**/
/*        IF _N_ = nobs THEN PUT / ' ])';*/
...
/*                ':cov_id  =>"', TRIM(cov_id)                        ,'", ',*/
/*                ':date => Date.new(',trim(put(year(date), 4.)), ',',*/
/*                                     trim(put(month(date),2.)),',',*/
/*                                     trim(put(day(date),  2.)),'), ',*/
/**/
/*                ':tobacco => ', IFC(tobacco = 'Y', 'true', 'false') , ', ',*/
/*                ':caffeine => ', IFC(caffeine = 'Y', 'true', 'false') , ', ',*/
/*                ':alcohol => ', IFC(alcohol = 'Y', 'true', 'false') , ', ',*/
/**/
/*                ':tobacco_num => ',  trim(put(tobacco_num,3.)), ', ',*/
/*                ':alcohol_num => ',  trim(put(alcohol_num,3.)), ', ',*/
/*                ':caffeine_num => ', trim(put(caffeine_num,3.)), ', ',*/
/**/
/**/
/*                ':med_ha => ', IFC(Med_HA = 'Y', 'true', 'false') , ', ',*/
/*                ':med_bp => ', IFC(Med_BP = 'Y', 'true', 'false') , ', ',*/
/*                ':med_can =>', IFC(Med_Can= 'Y', 'true', 'false') , ', ',*/
/*                ':med_diab=>', IFC(Med_Diab='Y', 'true', 'false') , ', ',*/
/*                ':med_chol=>', IFC(Med_Chol='Y', 'true', 'false') , ', ',*/
/*                ':med_arth=>', IFC(Med_Arth='Y', 'true', 'false') , ', ',*/
/*                ':med_asth=>', IFC(Med_Asth='Y', 'true', 'false') , ', ',*/
/*                ':med_gla =>', IFC(Med_Gla= 'Y', 'true', 'false') , ', ',*/
/*                ':med_kid =>', IFC(Med_Kid= 'Y', 'true', 'false') , ', ',*/
/*                ':med_leuk=>', IFC(Med_Leuk='Y', 'true', 'false') , ', ',*/
/*                ':med_ment=>', IFC(Med_Ment='Y', 'true', 'false') , ', ',*/
/*                ':med_se =>' , IFC(Med_SE = 'Y', 'true', 'false') , ', ',*/
/*                ':med_sca=>' , IFC(Med_SCA= 'Y', 'true', 'false') , ', ',*/
/*                ':med_str=>' , IFC(Med_Str= 'Y', 'true', 'false') , ', ',*/
/*                ':med_td =>' , IFC(Med_TD = 'Y', 'true', 'false') , ', ',*/
/*                ':med_tb =>' , IFC(Med_TB = 'Y', 'true', 'false') , ', ',*/
/*                ':med_ui =>' , IFC(Med_UI = 'Y', 'true', 'false') , '}, ');*/
/*RUN;*/
/**/
/*DATA _NULL_ ;*/
/*    * IF _N_ < 50 THEN DO;*/
/*        SET cust_med_seeds nobs=nobs; */
/*        FILE  'c:\iaa\export\seeds.rb' LRECL = 1050;*/
/*        IF _N_ = 1 THEN PUT 'custmeds = Custmed.create ([' ;*/
/**/
/*        PUT seed; */
/**/
/*        IF _N_ = nobs THEN PUT / ' ])';*/
...
/*RUN;*/
/**/
/*DATA transaction_seeds(KEEP = seed);*/
/*    FORMAT seed $800.;*/
/*    SET    customer_transactions;*/
/**/
/*    seed = cat(*/
/*                    '{:cust_id =>"', TRIM(cust_id),'", ',*/
/*                    ':cov_id => "', TRIM(cov_id), '", ',*/
/*                    ':transaction_code => "', TRIM(transaction), '", ',*/
/*                    ':transaction_type => "', TRIM(type),        '", ',*/
/*                    IFC(MISSING(Reward_R), ' ', cat(':reward_r => ', trim(put(reward_r,3.)), ', ')),*/
/*                    IFC(MISSING(Reward_A), ' ', cat(':reward_a => ', trim(put(reward_a,8.)), ', ')),*/
/**/
/*                    IFC(MISSING(cov_limit), ' ', cat(':cov_limit => ', TRIM(PUT(cov_limit,8.)),    ', ')),*/
/*                    IFC(MISSING(income), ' ', cat(':income => ',    TRIM(PUT(income,8.)),       ', ')),*/
/**/
/*                    IFC(MISSING(cov_income_ratio), ' ', cat(':cov_income_ratio => ',    TRIM(PUT(cov_income_ratio,6.2)),       ', ')),*/
/*                    */
/*                    IFC(MISSING(Transaction_date), ' ', cat(':transaction_date => Date.new(', trim(put(year(transaction_date),4.)), ',',*/
/*                                                                                              trim(put(month(transaction_date),2.)),',',*/
/*                                                                                              trim(put(day(transaction_date),2.)),  */
/*                                                                                          ') '*/
/*                                                           )*/
/*                       ),*/
/**/
/*                    IFC(~MISSING(adjudication_days) OR ~MISSING(adjuster_ID), ', ', ' '),*/
/**/
/*                    IFC(MISSING(adjudication_days), ' ',   cat(':adjudication_days => ',TRIM(PUT(adjudication_days,4.)),', ')),*/
/*                    IFC(MISSING(reward_cat),        ' ',   cat(':reward_cat => ',       TRIM(PUT(reward_cat,3.)),       ', ')),*/
/**/
/*                    IFC(MISSING(adjuster_id), ' ',   cat(':adjuster_id => ',  TRIM(adjuster_id),  ', ')),*/
/*                    IFC(MISSING(technician_id), ' ', cat(':technician_id => ',TRIM(technician_id),', ')),*/
/**/
/*                    IFC(MISSING(adj_city),  ' ', CAT(':adj_city => "',  trim(adj_city),  '", ')),*/
/*                    IFC(MISSING(adj_state), ' ', CAT(':adj_state => "', trim(adj_state), '" ')), '},'*/
/*                );*/
/*RUN;*/
/**/
/*DATA _NULL_ ;*/
/*    * IF _N_ < 50 THEN DO;*/
/*        SET transaction_seeds nobs=nobs; */
/*        FILE  'c:\iaa\export\seeds.rb' LRECL = 1050;*/
/*        IF _N_ = 1 THEN PUT 'transactions = Transaction.create ([' ;*/
/**/
/*        PUT seed; */
/**/
/*        IF _N_ = nobs THEN PUT / ' ])';*/
...
/*RUN;*/
/**/
/*DATA policies(KEEP= cust_id cov_id);*/
/*    SET customer_transactions;*/
/*    BY  cov_id;*/
/*    IF  FIRST.cov_id THEN OUTPUT;*/
/*RUN;*/
/**/
/*DATA pcheck;*/
/*    SET policies;*/
...
/*RUN;*/
/**/
/*DATA policy_seeds(KEEP = seed);*/
/*    FORMAT seed $200.;*/
/*    SET policies;*/
/**/
/*    seed = cat('{:cust_id =>"', TRIM(cust_id),'", ',*/
/*                ':cov_id => "', TRIM(cov_id), '"},');*/
/*RUN;*/
/**/
/*DATA _NULL_ ;*/
...
/*DATA customer_fm_seeds(KEEP = seed);*/
/*    FORMAT seed $1000.;*/
/*    SET customer_family_medical;*/
/**/
/*    seed = cat(*/
/*               '{:cust_id =>"', TRIM(cust_id)                       ,'", ',*/
/*                ':medf_ha => ', IFC(MedF_HA = 'Y', 'true', 'false') , ', ',*/
/*                ':medf_bp => ', IFC(MedF_BP = 'Y', 'true', 'false') , ', ',*/
/*                ':medf_can =>', IFC(MedF_Can= 'Y', 'true', 'false') , ', ',*/
/*                ':medf_diab=>', IFC(MedF_Diab='Y', 'true', 'false') , ', ',*/
/*                ':medf_chol=>', IFC(MedF_Chol='Y', 'true', 'false') , ', ',*/
/*                ':medf_arth=>', IFC(MedF_Arth='Y', 'true', 'false') , ', ',*/
/*                ':medf_asth=>', IFC(MedF_Asth='Y', 'true', 'false') , ', ',*/
/*                ':medf_gla =>', IFC(MedF_Gla= 'Y', 'true', 'false') , ', ',*/
/*                ':medf_kid =>', IFC(MedF_Kid= 'Y', 'true', 'false') , ', ',*/
/*                ':medf_leuk=>', IFC(MedF_Leuk='Y', 'true', 'false') , ', ',*/
/*                ':medf_ment=>', IFC(MedF_Ment='Y', 'true', 'false') , ', ',*/
/*                ':medf_se =>' , IFC(MedF_SE = 'Y', 'true', 'false') , ', ',*/
/*                ':medf_sca=>' , IFC(MedF_SCA= 'Y', 'true', 'false') , ', ',*/
/*                ':medf_str=>' , IFC(MedF_Str= 'Y', 'true', 'false') , ', ',*/
/*                ':medf_td =>' , IFC(MedF_TD = 'Y', 'true', 'false') , ', ',*/
/*                ':medf_tb =>' , IFC(MedF_TB = 'Y', 'true', 'false') , ', ',*/
/*                ':medf_ui =>' , IFC(MedF_UI = 'Y', 'true', 'false') , ', ',*/
/**/
/*                ':medm_ha => ', IFC(MedM_HA = 'Y', 'true', 'false') , ', ',*/
/*                ':medm_bp => ', IFC(MedM_BP = 'Y', 'true', 'false') , ', ',*/
/*                ':medm_can =>', IFC(MedM_Can= 'Y', 'true', 'false') , ', ',*/
/*                ':medm_diab=>', IFC(MedM_Diab='Y', 'true', 'false') , ', ',*/
/*                ':medm_chol=>', IFC(MedM_Chol='Y', 'true', 'false') , ', ',*/
/*                ':medm_arth=>', IFC(MedM_Arth='Y', 'true', 'false') , ', ',*/
/*                ':medm_asth=>', IFC(MedM_Asth='Y', 'true', 'false') , ', ',*/
/*                ':medm_gla =>', IFC(MedM_Gla= 'Y', 'true', 'false') , ', ',*/
/*                ':medm_kid =>', IFC(MedM_Kid= 'Y', 'true', 'false') , ', ',*/
/*                ':medm_leuk=>', IFC(MedM_Leuk='Y', 'true', 'false') , ', ',*/
/*                ':medm_ment=>', IFC(MedM_Ment='Y', 'true', 'false') , ', ',*/
/*                ':medm_se =>' , IFC(MedM_SE = 'Y', 'true', 'false') , ', ',*/
/*                ':medm_sca=>' , IFC(MedM_SCA= 'Y', 'true', 'false') , ', ',*/
/*                ':medm_str=>' , IFC(MedM_Str= 'Y', 'true', 'false') , ', ',*/
/*                ':medm_td =>' , IFC(MedM_TD = 'Y', 'true', 'false') , ', ',*/
/*                ':medm_tb =>' , IFC(MedM_TB = 'Y', 'true', 'false') , ', ',*/
/*                ':medm_ui =>' , IFC(MedM_UI = 'Y', 'true', 'false') , '},'*/
/*                );*/
/**/
/*RUN;*/
/**/
/*DATA _NULL_ ;*/
/*    * IF _N_ < 50 THEN DO;*/
/*        SET customer_fm_seeds ; */
/*        FILE  'c:\iaa\export\seeds.rb' LRECL = 1050;*/
/*        PUT seed; */
/*    * END;*/
/*RUN;*/

/* Some crazy shit to set up rails seeds */

/*DATA customer_seeds(KEEP = seed t_death_age death_age_group);*/
/*    FORMAT  seed  $1500.*/
/*            target $12.*/
/*            replacement $1.*/
/*            t_death_date_est $80.*/
/*            t_death_age $40.*/
/*            t_death_age_group $40.*/
/*            t_death_reward_r $40.*/
/*            t_death_reward_cat $40.*/
/*            t_death_adjuster $40.*/
/*            t_death_technician $40.*/
/*            t_death_adj_city $60.*/
/*            t_death_adj_state $40.*/
/*            t_death_cust_adj_dist $40.;*/
/*    SET customer_rails;*/
/**/
/*    IF fishy = 0     THEN t_fishy = 'false';     ELSE t_fishy = 'true';*/
/*    IF dead  = 0     THEN t_dead  = 'false';     ELSE t_dead  = 'true';*/
/*    IF valid_zip = 0 THEN t_valid_zip = 'false'; ELSE t_valid_zip = 'true';*/
/*    IF dup_ssn = 0   THEN t_dup_ssn = 'false';   ELSE t_dup_ssn = 'true';*/
/*    IF death_date_est = . THEN t_death_date_est = ' '; */
/*            ELSE t_death_date_est = ':death_date_est => Date.new(' || trim(put(year(death_date_est),4.)) || ','*/
/*                                                                     || trim(put(month(death_date_est),2.)) || ','*/
/*                                                                     || trim(put(day(death_date_est),2.)) || '), ';*/
/**/
/*    IF death_age = . THEN t_death_age = ' ';*/
/*            ELSE t_death_age = CATS(':death_age => ',trim(put(death_age,6.2)),', ');*/
/**/
/*    IF MISSING(death_age_group) THEN t_death_age_group = ' ';*/
/*            ELSE t_death_age_group = CATS(':death_age_group => "',trim(death_age_group),'", ');*/
/**/
/*    IF death_reward_r = . THEN t_death_reward_r = ' ';*/
/*            ELSE t_death_reward_r = CATS(':death_reward_r  => ',trim(put(death_reward_r,6.)),', ');*/
/**/
/*    IF death_reward_cat = . THEN t_death_reward_cat = ' ';*/
/*            ELSE t_death_reward_cat = CATS(':death_reward_cat => ',trim(put(death_reward_cat,3.)),', ');*/
/**/
/*    IF MISSING(death_adjuster) THEN t_death_adjuster = ' ';*/
/*            ELSE t_death_adjuster = CATS(':death_adjuster => ',trim(death_adjuster),', ');*/
/**/
/*    IF MISSING(death_technician) THEN t_death_technician = ' ';*/
/*            ELSE t_death_technician = CATS(':death_technician => ',trim(death_technician),', ');*/
/**/
/*    IF MISSING(death_adj_city) THEN t_death_adj_city = ' ';*/
/*            ELSE t_death_adj_city = CATS(':death_adj_city => "',trim(death_adj_city),'", ');*/
/**/
/*    IF MISSING(death_adj_state) THEN t_death_adj_state = ' ';*/
/*            ELSE t_death_adj_state = CATS(':death_adj_state => "',trim(death_adj_state),'", ');*/
/**/
/*    IF death_cust_adj_dist = . THEN t_death_cust_adj_dist = ' ';*/
/*            ELSE t_death_cust_adj_dist = CATS(':death_cust_adj_dist => ',trim(put(death_cust_adj_dist,8.)));*/
/**/
/*    seed =   '{:cust_id => "'                || trim(cust_id)  || '", '*/
/*           || ':name => "'                   || trim(name)     || '", '*/
/*           || ':fishy => '                   || trim(t_fishy)  || ', '*/
/*           || ':dead => '                    || trim(t_dead)   || ', '*/
/*           || ':gender => "'                 || trim(gender)   || '", '*/
...
/*           || ':city => "'                   || trim(city)     || '", '*/
/*           || ':state => "'                  || trim(state)    || '", '*/
/*           || ':customer_zip => "'           || trim(put(customerzip,Z5.)) || '", '*/
/*           || ':valid_zip => '               || trim(t_valid_zip) || ', '*/
/*           || ':cust_best_zip => "'          || trim(put(cust_best_zip,Z5.)) || '", '*/
/*           || ':telephoneNumber => "'        || trim(telephonenumber) || '", '*/
/*           || ':mothersmaiden => "'          || trim(mothersmaiden) || '", '*/
/*           || ':birthday => Date.new('       || trim(put(year(birthday),4.))  || ',' */
/*                                             || trim(put(month(birthday),2.)) || ',' */
/*                                             || trim(put(day(birthday),2.))   || '), '*/
/*           || ':customer_since => Date.new(' || trim(put(year(customer_since),4.))  || ',' */
/*                                             || trim(put(month(customer_since),2.)) || ',' */
/*                                             || trim(put(day(customer_since),2.))   || '), '*/
/**/
/*           || ':cctype => "'                 || trim(cctype) || '", '*/
/*           || ':ccexpires => Date.new('      || trim(put(year(ccexpires),4.))  || ',' */
/*                                             || trim(put(month(ccexpires),2.)) || ',' */
/*                                             || trim(put(day(ccexpires),2.))   || '), '*/
/*           || ':nationalid => "'             || trim(nationalid) || '", '*/
/*           || ':dup_ssn => '                 || trim(t_dup_ssn)  || ', '*/
/*           || ':bloodtype => "'              || trim(bloodtype) || '", '*/
/*           || ':pounds => '                  || trim(put(pounds,6.2)) || ', '*/
/*           || ':inches => '                  || trim(put(inches,3.))  || ', '*/
/*           || ':bmi => '                     || trim(put(bmi,5.2)) || ', '*/
/*           || ':bmi_code => '                || trim(put(bmi_code,3.)) || ', '*/
/*           || ':vehicle_year => '            || trim(put(vehicle_year,4.)) || ', '*/
/*           || ':vehicle_model => "'          || trim(vehicle_model) || '", '*/
/*           || ':cust_lat => '                || trim(put(cust_lat,6.2)) || ', '*/
/*           || ':cust_lon => '                || trim(put(cust_lon,6.2)) || ', '*/
/*           || ':cust_lat_rad => '            || trim(put(cust_lat_rad,6.2)) || ', '*/
/*           || ':cust_lon_rad => '            || trim(put(cust_lon_rad,6.2)) || ', '*/
/*           || ':bad_health_index => '        || trim(put(bad_health_index,5.2)) || ', '*/
/**/
/*           || ':tobacco_score => '           || trim(put(tobacco_score,5.2)) || ', '*/
/*           || ':caffeine_score => '          || trim(put(caffeine_score,5.2)) || ', '*/
/*           || ':alcohol_score => '           || trim(put(alcohol_score,5.2)) || ', '*/
/*           || ':grand_cov_limit => '         || trim(put(grand_cov_limit,8.)) || ', '*/
/*           || ':grand_reward_a => '          || trim(put(grand_reward_a,8.)) || ', '*/
/*           || ':grand_avg_cov_inc_ratio => ' || trim(put(grand_avg_cov_inc_ratio,6.3)) || ', '*/
/*           || ':city_id => '                 || trim(put(city_id,4.)) || IFC(dead=1,', ',' ')*/
/**/
/*           || trim(t_death_date_est)*/
/*           || trim(t_death_age)*/
/*           || trim(t_death_age_group)*/
/*           || trim(t_death_reward_r)*/
/*           || trim(t_death_reward_cat)*/
/*           || trim(t_death_adjuster)*/
/*           || trim(t_death_technician)*/
/*           || trim(t_death_adj_city)*/
/*           || trim(t_death_adj_state)*/
/*           || trim(t_death_cust_adj_dist)*/
/*           || '}, ';*/
/*           */
/*         ;*/
/*RUN;*/
/**/
/*/*proc export data=customer_seeds outfile="C:\iaa\export\seeds.csv"       */*/
/*/*replace; run;*/*/
/**/
/*/*PROC PRINT DATA = customer_seeds (OBS = 10);*/*/
/*/*RUN;*/*/
/**/
/*/*PROC EXPORT DATA = customer_seeds OUTFILE = 'C:\iaa\export\seeds.rb' DBMS = DLM;*/*/
/*/*    DELIMITER = ' ';*/*/
/*/*RUN;*/*/
/**/
/*DATA _NULL_ ;*/
/*    * IF _N_ < 50 THEN DO;*/
/*        SET customer_seeds ; */
/*        FILE  'c:\iaa\export\seeds.rb' LRECL = 1550;*/
/*        PUT seed; */
/*    * END;*/
/*RUN;*/
