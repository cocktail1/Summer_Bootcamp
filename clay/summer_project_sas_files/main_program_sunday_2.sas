...
DATA customer_transactions(DROP = t_age);
    RETAIN t_age;
    FORMAT t_age 4.2
           spread_years 4.2;
    ...
    ELSE
         DO;
            spread_years = Policy_Age_Years - t_age;
            t_age = Policy_Age_Years;
         END;
RUN;
...
DATA dem_by_zip(KEEP = cust_best_zip
                       count_dead
                       count_living
                       count_total
                       count_married
                       ...
                       count_black_dead
                       count_black_total
                       count_hispanic_living
                       count_hispanic_dead
                       count_hispanic_total
                       count_asian_living
                       count_asian_dead
                       count_asian_total
                       count_other_living
    ...
    RETAIN cust_best_zip
        ...
        FORMAT count_dead 8.
           ...
           count_single 8.
           count_dup_ssn 8.
           count_white_living 8.
           count_white_dead 8.
           count_white_total 8.
           count_black_living 8.
           count_black_dead 8.
           count_black_total 8.
           count_hispanic_living 8.
           count_hispanic_dead 8.
           count_hispanic_total 8.
           count_asian_living 8.
           count_asian_dead 8.
           count_asian_total 8.
           count_other_living 8.
           count_other_dead 8.
           count_other_total 8.
           total_income_ratio 8.
           mean_income_ratio 8.2
           total_policies 8.
           total_policies_rewarded 8.
           total_age_signup 8.
           mean_age_signup 8.2
           total_cat1_deaths 8.
            total_cat2_deaths 8.
    ...
    IF FIRST.cust_best_zip THEN 
      DO;
           ...
           count_single = 0;
           count_dup_ssn = 0;
           count_white_living = 0;
           count_white_dead = 0;
           count_white_total = 0;
           count_black_living = 0;
           count_black_dead = 0;
           count_black_total = 0;
           count_hispanic_living = 0;
           count_hispanic_dead = 0;
           count_hispanic_total = 0;
           count_asian_living = 0;
           count_asian_dead = 0;
           count_asian_total = 0;
           count_other_living = 0;
           count_other_dead = 0;
           count_other_total = 0;
           total_income_ratio = 0;
           mean_income_ratio = 0;
           total_policies = 0;
           total_policies_rewarded = 0;
           total_age_signup = 0;
           mean_age_signup = 0;
           total_cat1_deaths = 0;
           total_cat2_deaths = 0;
      ...
      IF dead = 1 THEN count_dead+1;
      IF dead = 0 THEN count_living+1;
      count_total+1;

      IF marriage = 'M' THEN count_married+1;
      IF marriage = 'W' THEN count_widowed+1;
      IF marriage = 'S' THEN count_single+1;
      IF dup_ssn  = 1   THEN count_dup_ssn+1;
      IF race = 'W' THEN count_white_total+1;
      IF race = 'W' & dead = 1 THEN count_white_dead+1;
      IF race = 'W' & dead = 0 THEN count_white_living+1;

      IF race = 'B' THEN count_black_total+1;
      IF race = 'B' & dead = 1 THEN count_black_dead+1;
      IF race = 'B' & dead = 0 THEN count_black_living+1;

      IF race = 'A' THEN count_asian_total+1;
      IF race = 'A' & dead = 1 THEN count_asian_dead+1;
      IF race = 'A' & dead = 0 THEN count_asian_living+1;

      IF race = 'H' THEN count_hispanic_total+1;
      IF race = 'H' & dead = 1 THEN count_hispanic_dead+1;
      IF race = 'H' & dead = 0 THEN count_hispanic_living+1;

      IF race = 'O' THEN count_other_total+1;
      IF race = 'O' & dead = 1 THEN count_other_dead+1;
      IF race = 'O' & dead = 0 THEN count_other_living+1;

      SELECT(death_reward_cat);
        WHEN(1) total_cat1_deaths+1;
        WHEN(2) total_cat2_deaths+1;
        WHEN(3) total_cat3_deaths+1;
        WHEN(4) total_cat4_deaths+1;
        WHEN(5) total_cat5_deaths+1;
        WHEN(6) total_cat6_deaths+1;
        WHEN(7) total_cat7_deaths+1;
        OTHERWISE;
      END;

      total_income_ratio = total_income_ratio + Grand_avg_Cov_Inc_Ratio;
      total_age_signup   = total_age_signup   + signup_age;
      total_bmi          = total_bmi          + bmi;

    total_age_death = SUM(total_age_death,death_age);

    IF Last.cust_best_zip THEN 
      DO;
        IF count_total > 0 THEN DO;
            mean_income_ratio = total_income_ratio / count_total;
            mean_age_signup   = total_age_signup / count_total;
            mean_bmi          = total_bmi / count_total;
        END;
        IF count_dead > 0 THEN DO;
            mean_age_death    = total_age_death / count_dead;
        END;
        OUTPUT;
      END;
RUN;
...
DATA dem_by_zip;
    RETAIN cust_best_zip;
    FORMAT living_dead_ratio 8.3 
           all_living 3.
           all_dead   3.
           excluded_totaldead_ratio 8.3;
    SET dem_by_zip;
    ...
    ELSE   
       DO;
            all_dead   = 0;
            all_living = 0;
            living_dead_ratio = count_living / count_dead;
       END;

    IF count_dead > 0 THEN excluded_totaldead_ratio = (count_dead - SUM(total_cat1_deaths,total_cat2_deaths,total_cat3_deaths)) / count_dead;
    ELSE excluded_totaldead_ratio = 0;

RUN;

PROC SORT DATA = dem_by_zip;
    BY  cust_best_zip;
RUN;

DATA pyr(KEEP = gender birthday current_age dead);
    FORMAT current_age 6.2;
    SET customer_consolidated;

    current_age = (TODAY() - birthday) / 365.2422;

RUN;

DATA pyr;
    FORMAT current_age_group $15.;
    SET pyr;

        SELECT;
        WHEN       (current_age < 20) current_age_group = '0-20';
        WHEN (20 <= current_age < 25) current_age_group = '20-25';
        WHEN (25 <= current_age < 30) current_age_group = '25-30';
        WHEN (30 <= current_age < 35) current_age_group = '30-35';
        WHEN (35 <= current_age < 40) current_age_group = '35-40';
        WHEN (40 <= current_age < 45) current_age_group = '40-45';
        WHEN (45 <= current_age < 50) current_age_group = '45-50';
        WHEN (50 <= current_age < 55) current_age_group = '50-55';
        WHEN (55 <= current_age < 60) current_age_group = '55-60';
        WHEN (60 <= current_age < 65) current_age_group = '60-65';
        WHEN (65 <= current_age < 70) current_age_group = '65-70';
        WHEN (70 <= current_age < 75) current_age_group = '70-75';
        WHEN (75 <= current_age < 80) current_age_group = '75-80';
        WHEN (80 <= current_age < 85) current_age_group = '80-85';
        WHEN (85 <= current_age < 90) current_age_group = '85-90';
        WHEN       (current_age > 90) current_age_group = '90 +';
    END;
RUN;

PROC SORT data = pyr;
    BY current_age_group;
RUN;
...
DATA pyr2;
    SET pyr(WHERE=(dead=0));
RUN;

TITLE 'CURRENT LIVING CUSTOMERS';
PROC FREQ DATA = pyr2;
    TABLES gender;
    BY current_age_group;
RUN;

DATA pyr3(KEEP= gender dead death_age_group);
    SET customer_consolidated(WHERE=(dead=1));
RUN;
...
/*DATA seeds_dem(KEEP = seed);*/
/*    FORMAT seed $1500.;*/
/*    SET    dem_by_zip;*/
/**/
/*    seed = CAT( '{:cust_best_zip => "', TRIM(PUT(cust_best_zip, Z5.)), '", ',*/
/**/
/*                 ':living_dead_ratio => ', IFC(living_dead_ratio = 0, '0', TRIM(PUT(living_dead_ratio,8.3))), ', ',*/
/**/
/*                 ':all_living => ', IFC(all_living = 1, 'true', 'false'), ', ',*/
/*                 ':all_dead => ', IFC(all_dead = 1, 'true', 'false'), ', ',*/
/**/
/**/
/*                 ':excluded_totaldead_ratio => ', IFC(excluded_totaldead_ratio = 0, '0', TRIM(PUT(excluded_totaldead_ratio, 8.3))), ', ',*/
/**/
/*                 ':count_dead => ', TRIM(PUT(count_dead,8.)), ', ',*/
/*                 ':count_living => ', TRIM(PUT(count_living, 8.)), ', ',*/
/*                 ':count_total => ', TRIM(PUT(count_total, 8.)), ', ',*/
/**/
/*                 ':count_married => ', TRIM(PUT(count_married, 8.)), ', ',*/
/*                 ':count_widowed => ', TRIM(PUT(count_widowed, 8.)), ', ',*/
/*                 ':count_single => ', TRIM(PUT(count_single, 8.)), ', ',*/
/*                 ':count_dup_ssn => ', TRIM(PUT(count_dup_ssn, 8.)), ', ',*/
/**/
/*                 ':count_white_living => ', TRIM(PUT(count_white_living, 8.)), ', ',*/
/*                 ':count_white_dead => ', TRIM(PUT(count_white_dead, 8.)), ', ',*/
/*                 ':count_white_total => ', TRIM(PUT(count_white_total, 8.)), ', ',*/
/**/
/*                 ':count_black_living => ', TRIM(PUT(count_black_living, 8.)), ', ',*/
/*                 ':count_black_dead => ', TRIM(PUT(count_black_dead, 8.)), ', ',*/
/*                 ':count_black_total => ', TRIM(PUT(count_black_total, 8.)), ', ',*/
/**/
/*                 ':count_hispanic_living => ', TRIM(PUT(count_hispanic_living, 8.)), ', ',*/
/*                 ':count_hispanic_dead => ', TRIM(PUT(count_hispanic_dead, 8.)), ', ',*/
/*                 ':count_hispanic_total => ', TRIM(PUT(count_hispanic_total, 8.)), ', ',*/
/**/
/*                 ':count_asian_living => ', TRIM(PUT(count_asian_living, 8.)), ', ',*/
/*                 ':count_asian_dead => ', TRIM(PUT(count_asian_dead, 8.)), ', ',*/
/*                 ':count_asian_total => ', TRIM(PUT(count_asian_total, 8.)), ', ',*/
/**/
/*                 ':count_other_living => ', TRIM(PUT(count_other_living, 8.)), ', ',*/
/*                 ':count_other_dead => ', TRIM(PUT(count_other_dead, 8.)), ', ',*/
/*                 ':count_other_total => ', TRIM(PUT(count_other_total, 8.)), ', ',*/
/**/
/*                 ':mean_income_ratio => ', TRIM(PUT(mean_income_ratio,8.2)), ', ',*/
/*                 ':total_policies => ', TRIM(PUT(total_policies, 8.)), ', ',*/
/*                 ':total_policies_rewarded => ', TRIM(PUT(total_policies_rewarded, 8.)), ', ',*/
/*                 */
/*                 ':mean_age_signup => ', TRIM(PUT(mean_age_signup, 8.2)), ', ',*/
/*                 */
/*                 ':total_cat1_deaths => ', TRIM(PUT(total_cat1_deaths, 8.)), ', ',*/
/*                 ':total_cat2_deaths => ', TRIM(PUT(total_cat2_deaths, 8.)), ', ',*/
/*                 ':total_cat3_deaths => ', TRIM(PUT(total_cat3_deaths, 8.)), ', ',*/
/*                 ':total_cat4_deaths => ', TRIM(PUT(total_cat4_deaths, 8.)), ', ',*/
/*                 ':total_cat5_deaths => ', TRIM(PUT(total_cat5_deaths, 8.)), ', ',*/
/*                 ':total_cat6_deaths => ', TRIM(PUT(total_cat6_deaths, 8.)), ', ',*/
/*                 ':total_cat7_deaths => ', TRIM(PUT(total_cat7_deaths, 8.)), ', ',*/
/**/
/*                 ':mean_bmi => ', TRIM(PUT(mean_bmi, 8.2)), ', ',*/
/*                 ':mean_age_death => ', IFC(mean_age_death = 0, '0', TRIM(PUT(mean_age_death, 4.2))), '},');*/
/*RUN;*/
/**/
/*DATA _NULL_ ;*/
/*    * IF _N_ < 50 THEN DO;*/
/*        SET seeds_dem nobs=nobs; */
/*        FILE  'c:\iaa\export\demographic_seeds.rb' LRECL = 1500;*/
/*        IF _N_ = 1 THEN PUT 'demographics = Demographic.create ([' ;*/
/**/
/*        PUT seed; */
/**/
...
/*RUN;*/
/**/
/*PROC SURVEYSELECT DATA     = customer_dead                   */
/*                  METHOD   = SRS */
/*                  REP      = 1 */
/*                  SAMPSIZE = 4900*/
/*                  SEED     = 8675309*/
...
/*RUN;*/
/**/
/*PROC GLM DATA = dead_sample plots(only)=diagnostics;*/
/*    MODEL death_age = signup_age; /*med_HA_score med_str_score med_can_score vehicle_year; */*/
/*RUN;*/
/*QUIT;*/
/**/
/*PROC CORR DATA = dead_sample rank plots(only MAXPOINTS=100000)=scatter(nvar=all ellipse=none);*/
/*    VAR death_age signup_age;*/
/*RUN;*/
